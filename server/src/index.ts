...
import { setupSockets } from './socket/handlers.js';
...
testRouter.get('/', (_req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      'POST /api/__test__/chat': '测试 AI 对话 - body: { "message": "你好" }',
      'POST /api/__test__/judge': '测试 AI 裁判 - body: { "guess": "推理", "solution": "真相", "surface?": "汤面" }',
      'POST /api/__test__/generate': '测试题目生成 - 无参数',
    },
    quickStart: {
      curl: 'curl -X POST http://localhost:3001/api/__test__/chat -H "Content-Type: application/json" -d \'{"message":"这碗汤的凶手是女人吗"}\'',
      powershell: 'Invoke-RestMethod -Uri http://localhost:3001/api/__test__/chat -Method POST -ContentType "application/json" -Body \'{"message":"这碗汤的凶手是女人吗"}\' | ConvertTo-Json',
    },
    codebuddyAI: {
      description: '在配置中开启测试模式即可使用 CodeBuddy 内置 AI（零配置，使用 IDE 积分）',
      status: '已集成 - 自动读取 IDE 认证 token',
      model: 'deepseek-v4-pro',
    },
  });
});

// GET /api/__test__/config — 查看当前完整配置（部分脱敏，仅测试用）
...
// POST /api/__test__/chat — 直接测试 AI 对话
...
// POST /api/__test__/judge — 直接测试 AI 裁判
